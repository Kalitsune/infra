...
- **Open WebUI**: SQLite is default, needs `local-path` PVC. Needs `WEBUI_SECRET_KEY` randomly generated into a SOPS secret. Pocket ID SSO needs `OAUTH_CODE_CHALLENGE_METHOD: S256` (PKCE) in extraEnvVars. Override `OPENAI_API_KEY` to `""` in extraEnvVars to clear the baked-in default when configuring custom `openaiBaseApiUrls`.
